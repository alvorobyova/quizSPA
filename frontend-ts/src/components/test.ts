...
export class Test {
  ...
  private startQuiz(): void {
    ...
    const preTitleElement: HTMLElement | null =
    ...
    if (this.prevButtonElement) {
      this.prevButtonElement.onclick = this.move.bind(
        this,
        ActionTestType.prev
      );
    }
  ...
  private prepareProgressBar(): void {
    ...
    for (let i = 0; i < this.quiz.questions.length; i++) {
      ...
      const itemCircleElement: HTMLElement | null =
  ...
  private showQuestion(): void {
    ...
    const activeQuestion: QuizQuestionType =
  ...
  private move(action: ActionTestType): void {
    ...
    if (this.progressBarElement) {
      Array.from(this.progressBarElement.children).forEach(
  ...
  private async complete(): Promise<void> {
    ...
    try {
      const result: DefaultResponseType | PassTestResponseType =
